...
## 优化视频质量与比特率大小
在使用`bit_rate`与`avg_quality`调整画面质量与比特率大小限制时，两个参数配置作用相互互斥。调整了优先级来更精确的控制平衡比特率和质量

在默认情况下，`KPlayer`会选择较为均衡的方式来处理画质与比特率的大小。`bit_rate`与`avg_quality`参数功能其中只能有一个参数生效
在同时设置时`bit_rate`的优先级会更高
